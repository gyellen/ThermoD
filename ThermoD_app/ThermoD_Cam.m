function varargout = ThermoD_Cam(varargin)
% THERMOD_CAM MATLAB code for ThermoD_Cam.fig
%      THERMOD_CAM, by itself, creates a new THERMOD_CAM or raises the existing
%      singleton*.
%
%      H = THERMOD_CAM returns the handle to a new THERMOD_CAM or the handle to
%      the existing singleton*.
%
%      THERMOD_CAM('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in THERMOD_CAM.M with the given input arguments.
%
%      THERMOD_CAM('Property','Value',...) creates a new THERMOD_CAM or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ThermoD_Cam_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ThermoD_Cam_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ThermoD_Cam

% Last Modified by GUIDE v2.5 29-Jun-2021 10:43:32

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ThermoD_Cam_OpeningFcn, ...
                   'gui_OutputFcn',  @ThermoD_Cam_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ThermoD_Cam is made visible.
function ThermoD_Cam_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ThermoD_Cam (see VARARGIN)

% Choose default command line output for ThermoD_Cam
...
if isempty(ThermCam) 
    ...
    ThermCam = ThermalCamera.getInstance;
    ThermCam.initIPC;
end
...
if ~isfield(Thermo,'trackPhy')
    Thermo.trackPhy = false;
    Thermo.cbTrackPhy.Value = Thermo.trackPhy;    
end

if ~isfield(Thermo,'avoidCoolPhy') || ~Thermo.DXmonAmps1
    Thermo.avoidCoolPhy = false;
    Thermo.cbAvoidCoolPhy.Value = Thermo.avoidCoolPhy;    
end
...
% set up for stimulation 
% stimulator setup using NI output on NI 'Dev1'
global hDAQ stim 
if ~isfield(Thermo,'nostim')
    if ~isempty(hDAQ), hDAQ = []; end    
    evalin('base','global hDAQ stim');
    %     stim = zeros(10000,1);
    %     stim(1+100*(0:99)) = 5;
    % create the NI session with one output channel
    hDAQ = daq.createSession('ni'); %  (We use the NI USB-6001)
    hDAQ.Rate = 5000;  % 5000 points per second
    % stimulus is 5000/sec, Thermo.stim sec, pulses of 0.2 ms @ 50Hz
    Thermo.stimlen = str2double(get(handles.eStimLength,'String'));
    Thermo.stimpause = str2double(get(handles.eStimPause,'String'));
    Thermo.stimrepeat = str2double(get(handles.eStimRepeat,'String'));
    stim = create_stim(Thermo.stimlen, 5000);  
...
function varargout = ThermoD_Cam_OutputFcn(hObject, eventdata, handles) 
...
timedMsg(['Initiating physiology with setpoint: ' sprintf('%4.1f',Thermo.tPhysiol)]);
...
timeTicAdj  = 1000; %  ms  (shortest repeat time for autoregulatory control)
...
if tTic1 >= timeTicSlow
    ...
    if Thermo.DXtelemCode 
       ...
       if Thermo.DXmonTemp1
           ...
           tt(1:end-1) = 10*tt(1:end-1); % scale current and voltage by 10
       else
           tt(1:end) = 10*tt(1:end); % scale current and voltage by 10
       end
       Tblock = [Tblock tt(:)'];
    end
...
if tTicA >= timeTicAdj
    ...
    if Thermo.mode==1
        % if we are avoiding cooling in Physiology, check the TEC1 current
        if Thermo.avoidCoolPhy && Thermo.DXmonAmps1
            % tThermistor is in the last position (if measured)
            % current is in the position before that
            TEC1_current = Thermo.tExtra(end-Thermo.DXmonTemp1); % no temp, so current is last
        else
            TEC1_current = 0;
        end
        TEC1_resistance = 0.9; % ohms (nominal from spec sheet)
        TEC1_adjustVolt = TEC1_current*TEC1_resistance;
        ...
        if tTEC1>42 || tBlock>42
            ...
            if Thermo.trackPhy % we should adjust Vset to be the camera-measured temp
                % use the LAST roi, usually the slice position
                TtoTrack  = Thermo.camTemps(end,Thermo.fastIdx); 
                if TtoTrack>32 && TtoTrack<37
                    Tset = TtoTrack; % doesn't get stored, so when we turn off it reverts to GUI value
                end
            end
            Vprev = Vset;
            if isnan(Vset) || Vset<0 || tTEC1>(Tset+1) || Thermo.noFBPhy % || (Thermo.strongPhy && tTEC1<(Tset-3))
                ...
                Vset = Vset + TEC1_adjustVolt; % adjust if we have slipped to cooling
            else
...
idx1 = Thermo.fastIdx;
idx2 = Thermo.slowIdx;
Thermo.handles.tTEC1.String  = ['T(cam1) = '  num2str(Thermo.camTemps(1,idx1),'%5.1f') '°C'];
...
% --- Executes on button press in cbTrackPhy.
function cbTrackPhy_Callback(hObject, eventdata, handles)
global Thermo
Thermo.trackPhy = hObject.Value;

% --- Executes on button press in cbAvoidCoolPhy.
function cbAvoidCoolPhy_Callback(hObject, eventdata, handles)
global Thermo
if Thermo.DXmonAmps1
    Thermo.avoidCoolPhy = hObject.Value;
else
    hObject.Value = false; % can't do this unless monitoring current
end

function pbStim_Callback(hObject, eventdata, handles)
global hDAQ stim Thermo
for k = 1:Thermo.stimrepeat
    hDAQ.queueOutputData(stim);
    timedMsg('Stimulated');
    hDAQ.startForeground;
    pause(Thermo.stimpause); % in sec
end

function eStimLength_Callback(hObject, eventdata, handles)
global Thermo stim
Thermo.stimlen = str2double(get(handles.eStimLength,'String'));
stim = create_stim(Thermo.stimlen, 5000);

function eStimLength_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end
...
function eStimRepeat_Callback(hObject, eventdata, handles)
global Thermo
Thermo.stimrepeat = str2double(get(handles.eStimRepeat,'String'));

function eStimRepeat_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end


function eStimPause_Callback(hObject, eventdata, handles)
global Thermo
Thermo.stimpause = str2double(get(handles.eStimPause,'String'));

function eStimPause_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end
