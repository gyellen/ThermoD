...
struct FrameMetadata
{
	...
	int FlagState;  // gy changed from TFlagState FlagState;
	float TempChip;
	float TempFlag;
	float TempBox;
	WORD PIFin[2];			
...
IMAGERIPC_API HRESULT WINAPI GetFrame(WORD index, WORD timeout, void *pBuf, unsigned int  Size, void *pMetadata); //Get frame with metadata structure 1
IMAGERIPC_API HRESULT WINAPI GetVisibleFrame(WORD index, WORD timeout, void *pBuf, unsigned int Size, void *pMetadata); //Get frame with metadata structure 2
